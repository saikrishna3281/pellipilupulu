...
def get_upload_to(instance, filename):
    reg_id = str(instance.reg_id)
    print(str(reg_id))
    return f'/media/{reg_id}/{filename}'
...
class MatrimonyProfile(models.Model):
    ...
    profile_pic = models.ImageField(upload_to=get_upload_to, blank = True, null=True)
    is_nri = models.IntegerField(blank=True, null=True)
...
class Occupation(models.Model):
    occupation = models.CharField(unique=True, max_length=50)

    class Meta:
        db_table = 'occupation'

class IsNRI(models.Model):
    is_nri = models.CharField(unique=True, max_length=50)
